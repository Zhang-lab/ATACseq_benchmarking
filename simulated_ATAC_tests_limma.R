...
for (i in c(20,15,12,10,8,6,5,4,3,2))
{
  ...
  mylimma <- DGEList(mydata)
  mylimma <- calcNormFactors(mylimma)
  myvoom <- voom(mylimma, mydesign)
  myfit <- lmFit(myvoom, mydesign)
  myfit <- eBayes(myfit)
  mypadj <- p.adjust(myfit$p.value[,2], method = 'BH')
  myresults <- as.data.frame(mypadj)
  myresults$names <- rownames(mydata)
  colnames(myresults) <- c("FDR","Names")
  rownames(myresults) <- rownames(mydata)
  ...
  write.table(results,file=paste("limma_",basename(args[1]),"_",i,"_reps.txt",sep = ""),quote = FALSE, sep = )
}